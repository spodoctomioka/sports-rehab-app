  ...
  const data: PhaseData[] = [
    {
      ...
      rehabMenu: [
        ...
        { title: "体幹安定化",           sets: "30秒 × 3セット", note: "ドローイン、プランク（疼痛なし）", details: "ドローインは腹横筋を意識して腹部を軽く凹ませる運動です。プランクは四肢・腹部で体を支え30秒キープします。患部への負荷はほぼゼロで体幹の安定性を維持できます。体幹の安定性が不足したまま競技復帰すると再受傷リスクが上昇することが知られています。" },
      ],
      timeline: [
    ...
    },
    {
      ...
      rehabMenu: [
        ...
        { title: "スクワット・レッグプレス",     sets: "10回 × 4",    note: "フルレンジ。体重1.5倍を目標", details: "フルレンジ（膝90度以上）のスクワットを行います。体重の1〜1.5倍の負荷が目標ですが、疼痛のない範囲内のみで実施してください。ハーフスクワットから始め、段階的にフルレンジへ移行します。レッグプレスは膝が内側に入らないようアライメントに注意してください。" },
  ...
  return {
    ...
    clinicalGuidance: p.injuryId === "hamstring"
      ? `Hickey JT, et al. Sports Med 2017;47:1375-1387（ハムストリングス肉離れRTP基準：システマティックレビュー）\n` +
        `■ Askling H-test（能動的SLR：伸張感出現位置が健側比95%以上）を用いた研究では再受傷率1.3〜3.6%（各研究中最低値）\n` +
