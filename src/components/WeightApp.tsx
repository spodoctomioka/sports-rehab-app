...
function ManagerBulkScreen({players,onSave,onBack}:{players:Player[];onSave:(u:Player[])=>void;onBack:()=>void;}){
  ...
  const hasTeams=players.some(p=>p.team);
  const thuUnmeasuredCount=players.filter(p=>isThursdayUnmeasured(p)).length;
  const baseSorted=sortMode==="team"?sortByTeamAndGrade(players):sortByGrade(players);
  const sorted=sortMode==="thursday"
    ?[...players].sort((a,b)=>{const ua=isThursdayUnmeasured(a)?0:1,ub=isThursdayUnmeasured(b)?0:1;if(ua!==ub)return ua-ub;const ga=calcGrade(a.birthDate),gb=calcGrade(b.birthDate);if(ga!==gb)return gb-ga;return a.name.localeCompare(b.name,"ja");})
    :baseSorted;
